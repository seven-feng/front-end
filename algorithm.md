...
function knapsack(weight, value, w, n) {
  ...
  for(let i = 0; i < n; i++) {
    for(let j = w; j >=weight[i]; j--) { // 从后往前，可以防止重复计算；从前往后的话，如果前面加了，后面会在前面的基础上继续加，导致了重复计算。
...
根据所包含的编辑操作种类的不同，编辑距离有多种不同的计算方式，比较著名的有**莱文斯坦距离**（Levenshtein distance）和**最长公共子串长度**（Longest common substring length）。其中，莱文斯坦距离允许**增加、删除、替换字符**这三个编辑操作，~~最长公共子串长度只允许**增加、删除字符**这两个编辑操作~~（这里容易产生误导，感觉两个还是不同的东西）。
